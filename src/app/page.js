...
// ── ▼▼▼ FONT_OPTS：繊細系3種 ＋ 太字系5種 ＋ ポップ系3種に拡張 ▼▼▼ ──────────
const FONT_OPTS = [
  // 繊細系
  { id:"cormorant", label:"Cormorant",   tag:"エレガント",    type:"thin", css:"'Cormorant Garamond',serif",        weight:300 },
  { id:"noto",      label:"Noto Sans",   tag:"スタンダード",  type:"thin", css:"'Noto Sans JP',sans-serif",          weight:300 },
  { id:"notoserif", label:"Noto Serif",  tag:"明朝",          type:"thin", css:"'Noto Serif JP',sans-serif",         weight:300 },
  { id:"maru",      label:"丸ゴシック",  tag:"やわらか",      type:"thin", css:"'M PLUS Rounded 1c',sans-serif",    weight:300 },
  { id:"zenantiq",  label:"Zen Antique", tag:"アンティーク",  type:"thin", css:"'Zen Antique',serif",                weight:400 },
  { id:"kaisei",    label:"Kaisei Decol",tag:"デコール",      type:"thin", css:"'Kaisei Decol',serif",               weight:400 },
  // 太字系
  { id:"notobold",  label:"Noto Bold",   tag:"太字",          type:"bold", css:"'Noto Sans JP',sans-serif",          weight:700 },
  { id:"noto900",   label:"Noto Black",  tag:"極太",          type:"bold", css:"'Noto Sans JP',sans-serif",          weight:900 },
  { id:"zenmaru7",  label:"Zen Bold",    tag:"丸ゴシ太",      type:"bold", css:"'Zen Maru Gothic',sans-serif",       weight:700 },
  { id:"biz",       label:"BIZ UDゴシ",  tag:"UDゴシ太",      type:"bold", css:"'BIZ UDPGothic',sans-serif",         weight:700 },
  { id:"dela",      label:"Dela Gothic", tag:"極太ゴシ",      type:"bold", css:"'Dela Gothic One',sans-serif",       weight:400 },
  // ポップ系
  { id:"rounded",   label:"M PLUS",      tag:"ポップ",        type:"pop",  css:"'M PLUS Rounded 1c',sans-serif",    weight:400 },
  { id:"rocknroll", label:"RocknRoll",   tag:"ロック",        type:"pop",  css:"'RocknRoll One',sans-serif",         weight:400 },
  { id:"dot",       label:"DotGothic",   tag:"ドット",        type:"pop",  css:"'DotGothic16',sans-serif",           weight:400 },
  { id:"rampart",   label:"Rampart",     tag:"ポップ枠",      type:"pop",  css:"'Rampart One',sans-serif",           weight:400 },
];

// ── ▼▼▼ カラーパレット 36色 ▼▼▼ ─────────────────────────────────────────────
const COLOR_PALETTE = [
  // ホワイト系
  "#ffffff","#f0f0f0","#e8e8f8","#ffe4f0",
  // パステル
  "#fde68a","#fca5a5","#f9a8d4","#c4b5fd","#93c5fd","#6ee7b7",
  // ビビッド
  "#fbbf24","#f97316","#ef4444","#ec4899","#a855f7","#6366f1","#3b82f6","#10b981",
  // 占い系（紫・金・月）
  "#ffe0ff","#e0c8ff","#c8a0ff","#ffd700","#b8860b","#f0e0c0",
  // ダーク
  "#1f2937","#111827","#0f172a","#000000",
  // Serena系
  "#e8d5f5","#c084fc","#d4b483","#b8962e",
  // ヒーリング系
  "#d0ffe0","#a0ffd0","#c8ffc8","#e0f0ff",
];
...
function drawFrame(ctx, slide, frame, totalF, fxState, qrImg, logoImg, global, scale=1) {
  ...
  // ▼ フォントCSS・ウェイトをFONT_OPTSから取得
  const fontDef = FONT_OPTS.find(f=>f.id===slide.font) || FONT_OPTS[0];
  const fontCss = fontDef.css;
  const fontWeight = fontDef.weight;
  ...
  if(slide.textVertical) {
    ...
    ctx.font=`${fontWeight} ${fs}px ${fontCss}`;
    ctx.fillStyle=hexRgba(slide.textColor,alpha);
...
function SlideVideoApp() {
  ...
  const [fontTab, setFontTab] = useState("thin");
  ...
  // ── ▼▼▼ フォントタブでフィルタ ▼▼▼ ──
  const filteredFonts = fontTab === "all" ? FONT_OPTS : FONT_OPTS.filter(f => f.type === fontTab);
        ...
        <div style={{flex:"1 1 340px",maxWidth:430,display:"flex",flexDirection:"column",gap:14}}>
          ...
          {tab==="text"&&(
            <Panel title="✍ テキスト設定">
              ...
              {/* ── ▼▼▼ フォント選択（タブ＋グリッド）▼▼▼ ── */}
              <Row label="フォント">
                <div style={{display:"flex",gap:3,background:"rgba(0,0,0,0.3)",borderRadius:8,padding:3,marginBottom:8}}>
                  {[["thin","繊細系"],["bold","太字系"],["pop","ポップ系"],["all","すべて"]].map(([t,l])=>(
                    <button key={t} onClick={()=>setFontTab(t)} style={{flex:1,padding:"4px 0",fontSize:10,fontWeight:"bold",borderRadius:5,cursor:"pointer",border:"none",fontFamily:"inherit",
                      background:fontTab===t?"rgba(160,80,255,0.35)":"transparent",
                      color:fontTab===t?"#e0c0ff":"#8070a0"}}>
                      {l}
                    </button>
                  ))}
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5,maxHeight:240,overflowY:"auto"}}>
                  {filteredFonts.map(f=>(
                    <div key={f.id} onClick={()=>updateSlide(activeSlide,{font:f.id})}
                      style={{background:sl.font===f.id?"rgba(160,80,255,0.25)":"rgba(255,255,255,0.04)",
                        border:`1.5px solid ${sl.font===f.id?"#c080ff":"rgba(180,140,255,0.15)"}`,
                        borderRadius:8,padding:"7px 8px",cursor:"pointer",textAlign:"center"}}>
                      <div style={{fontFamily:f.css,fontWeight:f.weight,fontSize:18,color:"#e8d8ff",marginBottom:2,lineHeight:1.2}}>あAa</div>
                      <div style={{fontSize:10,color:"#9080b0",marginBottom:2}}>{f.label}</div>
                      <div style={{fontSize:9,color:"#c080ff",background:"rgba(160,80,255,0.15)",borderRadius:3,padding:"1px 5px",display:"inline-block"}}>{f.tag}</div>
                    </div>
                  ))}
                </div>
              </Row>
              ...
              <Row label="文字色">
                <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:6}}>
                  {COLOR_PALETTE.map(c=>(
                    <div key={c} onClick={()=>updateSlide(activeSlide,{textColor:c})}
                      style={{width:24,height:24,borderRadius:5,background:c,cursor:"pointer",flexShrink:0,
                        border:sl.textColor===c?"2px solid #fff":"2px solid transparent",
                        boxShadow:sl.textColor===c?"0 0 0 1px #c080ff":"none",
                        transform:sl.textColor===c?"scale(1.15)":"scale(1)",transition:"transform .1s"}}/>
                  ))}
                </div>
                <div style={{display:"flex",gap:7,alignItems:"center"}}>
                  ...
                  <span style={{fontSize:11,opacity:0.5}}>カスタムカラー</span>
                  <span style={{fontSize:11,fontFamily:"monospace",color:"#c080ff"}}>{sl.textColor}</span>
                </div>
              </Row>
